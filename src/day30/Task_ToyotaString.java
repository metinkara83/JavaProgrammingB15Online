...
public class Task_ToyotaString {
...
//        put all toyota cars in new array called toyotaInventory
//        Hint : first count toyota to find out your new array size then fill it up with toyota cars

        int toyotaCount = 0;
        for (String eachCar : cars) {
            if (eachCar.toLowerCase().contains("toyota")){
                toyotaCount++;
            }
        }
        System.out.println("Toyota brand car number is : "+toyotaCount);

        String[] toyotaInventory = new String[toyotaCount];
        int j = 0;
        for (int i = 0; i < cars.length ; i++) {
            if (cars[i].toLowerCase().contains("toyota")){
                toyotaInventory[j] = cars[i].substring(cars[i].indexOf("-")+1);
                j++;
            }
        }
